...
module.exports = function init(opts = {}) {
  ...
  // Otherwise, go straight to the transform function.
    ...
    const stream = transform.apply(this);
    return stream;
  }
  ...
  function transform() {
    ...
    return through.obj((file, enc, cb) => {
      ...
      this.parse(file, opts).then(data => {
        // Change the extension of the incoming file, and replace the Markdown contents with rendered HTML
        file.path = replaceExt(file.path, `.${this.options.extension}`);
