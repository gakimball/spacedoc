...
module.exports = function(file, opts = {}) {
  ...
  page.__fm = Object.assign({}, pageData.attributes);
  page.docs = '';
  ...
  if (this.options.marked) {
    try {
      page.docs = marked(pageData.body, { renderer: this.options.marked });
    }
    catch (e) {
  ...
  const parsers = Object.keys(this.adapters).filter(lib => lib in page).map(lib => {
    ...
    const config = Object.assign(this.adapters[lib].config, this.options.config[lib] || {});

    return new Promise((resolve, reject) => {
      this.adapters[lib](page[lib], config)
